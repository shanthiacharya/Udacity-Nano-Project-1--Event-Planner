...
function initDateTime() {
  ...
  $( "#InputEventstarttime" ).timepicker();
  $( "#InputEventendtime" ).timepicker();
}
...
function getEventsList (){
  ...
  var eventList = snap.val();
         ...
         $.each( eventList, function( i, val ) {
              //  for( key in val ) {
                  //console.log(key);
                 $("ul#mylist").append("<li class='list-group-item'>" +"<i>" + val['startdate' ] + " "+ val['starttime' ] +"</i>" +  "<h3>"+  val['title' ] + "</h3>" + "<p>" + "From: " + val['hostname'] +"</p>"  +"</li>");
              //  } // END for
          }); // END each
      }  // END if snap.val
    else{
